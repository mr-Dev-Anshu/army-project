...
import { useCreateMobilePhoneInOutRegister, useUpdateMobilePhoneInOutRegister } from "../hooks";
import { toast } from "react-toastify";
import { formatDateForInput } from "@/utils/dateUtils";
import { SuggestionInput } from "@/common/component/SuggestionInput";

interface MobilePhoneInOutFormProps {
    initialData?: any;
    onSuccess?: () => void;
    onCancel?: () => void;
}

const MobilePhoneInOutForm = ({ initialData, onSuccess, onCancel }: MobilePhoneInOutFormProps) => {
    const createMutation = useCreateMobilePhoneInOutRegister();
    const updateMutation = useUpdateMobilePhoneInOutRegister();
    ...
    const [mobilePhoneOutTime, setMobilePhoneOutTime] = useState("");
    const [outSignature, setOutSignature] = useState("");
    const [dateOfDuty, setDateOfDuty] = useState(new Date().toISOString().split('T')[0]);
    const [placeOfDuty, setPlaceOfDuty] = useState("");
    const [typeOfDuty, setTypeOfDuty] = useState("");
    const [mobilePhoneInTime, setMobilePhoneInTime] = useState("");
    ...
    useEffect(() => {
        if (initialData) {
            ...
            setMobilePhoneOutTime(d.mobilePhoneOutTime || "");
            setOutSignature(d.outSignature || "");
            setPlaceOfDuty(d.placeOfDuty || "");
            setTypeOfDuty(d.dutyType || "");
            setMobilePhoneInTime(d.mobilePhoneInTime || "");
        ...
        <div className="mx-auto w-full max-w-4xl font-inter">
            <div className="space-y-8">
                ...
                <section className="space-y-4">
                    <h3 className="text-sm font-bold text-neutral-900">Mobile Phone OUT Details</h3>
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div className="space-y-1.5">
                            <Label htmlFor="dutyOutTime" className="text-sm font-medium text-neutral-700">
                                Mobile Phone OUT Time
                            </Label>
                            <Input
                                id="mobilePhoneOutTime"
                                type="time"
                                className="block w-full"
                                value={mobilePhoneOutTime}
                                onChange={(e) => setMobilePhoneOutTime(e.target.value)}
                            />
                        </div>
                        <div className="space-y-1.5">
                ...
                <section className="space-y-4 pt-4 border-t border-neutral-100">
                    ...
                    {/* Date/Time fields removed to match UI design */}
                    <div className="space-y-1.5">
                        <SuggestionInput
                            label="Place of Duty"
                ...
                {/* Duty IN Details */}
                <section className="space-y-4 pt-4 border-t border-neutral-100">
                    <h3 className="text-sm font-bold text-neutral-900">Mobile Phone IN Details</h3>
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div className="space-y-1.5">
                            <Label htmlFor="mobilePhoneInTime" className="text-sm font-medium text-neutral-700">
                                Mobile Phone IN Time
                            </Label>
                            <Input
                                id="mobilePhoneInTime"
                                type="time"
                                className="block w-full"
                                value={mobilePhoneInTime}
                                onChange={(e) => setMobilePhoneInTime(e.target.value)}
                            />
                        </div>
                        <div className="space-y-1.5">
...
export default MobilePhoneInOutForm;
