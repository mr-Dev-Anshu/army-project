...
interface ContactInfoArmyPersonnelTableProps {
    data: any[];
...
const ContactInfoArmyPersonnelTable = ({ data, onEdit, onDelete, onAddNew }: ContactInfoArmyPersonnelTableProps) => {
    ...
    const filteredData = useMemo(() => {
        return data.filter((item) => {
            ...
            const matchesSearch =
                ...
                (d.unit?.toLowerCase() || "").includes(searchTerm) ||
                (d.postedAt?.toLowerCase() || "").includes(searchTerm) ||
                (d.mobileNumber?.toLowerCase() || "").includes(searchTerm) ||
                (d.appointment?.toLowerCase() || "").includes(searchTerm);

            // Filter logic
            const matchesAppointment = filters.appointment
                ? (d.appointment?.toLowerCase() || "").includes(filters.appointment.toLowerCase())
                : true;

            const matchesRank = filters.rank
                ? (d.rank?.toLowerCase() || "").includes(filters.rank.toLowerCase())
                : true;

            const matchesUnit = filters.unit
                ? (d.unit?.toLowerCase() || "").includes(filters.unit.toLowerCase())
                : true;

            const matchesLocation = filters.postingLocation
                ? (d.postedAt?.toLowerCase() || "").includes(filters.postingLocation.toLowerCase())
                : true;

            return matchesSearch && matchesAppointment && matchesRank && matchesUnit && matchesLocation;
        });
    }, [data, filters]);
    ...
    const rankOptions = useMemo(() => {
        const ranks = new Set(data.map(item => item.details?.rank).filter(Boolean));
        return Array.from(ranks) as string[];
    }, [data]);

    const unitOptions = useMemo(() => {
        const units = new Set(data.map(item => item.details?.unit).filter(Boolean));
        return Array.from(units) as string[];
    }, [data]);

    const appointmentOptions = useMemo(() => {
        const appointments = new Set(data.map(item => item.details?.appointment).filter(Boolean));
        return Array.from(appointments) as string[];
    }, [data]);

    const postingLocationOptions = useMemo(() => {
        const locations = new Set(data.map(item => item.details?.postedAt).filter(Boolean));
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <g clip-path="url(#clip0_1140_118731)">
                        ...
                        <defs>
                            <clipPath id="clip0_1140_118731">
                                <rect width="12" height="12" fill="white" transform="translate(6 6)" />
                            </clipPath>
                        </defs>
                    </svg>

                    <h2 className="text-lg font-semibold text-[#404040]">Contact Info. of Army Personnel & their Appointment</h2>
                </div>
            ...
            <ReportFilterBar
                ...
                showPostingLocation={true}
                postingLocationOptions={postingLocationOptions}
                ...
                onAddNew={onAddNew}
                placeholder="Search by Name, Rank, Unit, Appointment..."
            />
                ...
                <div className="overflow-auto max-h-[75vh] relative [&::-webkit-scrollbar]:hidden">
                    <table className="w-full text-left text-sm text-gray-700 min-w-[1200px]">
                        <thead className="bg-[#F5F5F5] text-xs font-bold text-gray-900 border-b border-gray-300">
                            <tr>
                                <th rowSpan={2} className="px-4 py-3 border-r border-gray-300 w-16 align-middle sticky left-0 top-0 z-50 bg-[#F5F5F5]">
                                    Sr no.
                                </th>
                                <th rowSpan={2} className="px-4 py-3 border-r border-gray-300 w-24 align-middle sticky top-0 z-40 bg-[#F5F5F5]">
                                    Rank
                                </th>
                                ...
                                <th rowSpan={2} className="px-2 py-3 w-10 text-center align-middle sticky right-0 top-0 z-50 bg-[#F5F5F5] border-l border-gray-300"></th>
                            </tr>
                            <tr>
...
export default ContactInfoArmyPersonnelTable;
