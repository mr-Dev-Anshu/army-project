...
export default function VehicleDetailsForm({
  ...
  const [hasArmyRelative, setHasArmyRelative] = useState(
    Boolean(relativeType),
  );

  /* 🔥 sync local → context */
  useEffect(() => {
    setHasArmyRelative(Boolean(relativeType));
  }, [relativeType]);
    ...
    <div className="bg-white p-4 space-y-6">
      ...
      <div>
        ...
        <RadioGroup
          ...
          className="grid sm:grid-cols-2 gap-3"
        >
          {["Civilian Vehicle", "DD Vehicle"].map((v) => (
            <label
              key={v}
              className={cn(
                "border rounded-lg px-4 py-2 flex gap-2 cursor-pointer",
