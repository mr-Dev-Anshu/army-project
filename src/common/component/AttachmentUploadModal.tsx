...
export default function AttachmentUploadModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 🔥 IMPORTANT: files is already File[]
      const uploadResults = await uploadMultipleFiles(files);

      const attachments: AttachedItem[] = uploadResults.map(
        (res: any, idx: number) => {
          let finalName = files[idx]?.name || "Attachment";

          if (prefixName.trim()) {
            finalName =
              files.length > 1 ? `${prefixName} ${idx + 1}` : prefixName;
          }
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="flex gap-3">
            {(["Certificate", "Forms", "Letter"] as AttachmentType[]).map(
              (item) => (
                <button
                  key={item}
        ...
        <div className="rounded-xl">
          <FileDropZone
            onFileSelect={(newFiles: File | File[]) => {
