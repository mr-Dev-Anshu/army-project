...
// Define a type for your menu items to allow proper typing below
type MenuItem = {
  icon?: ReactNode;
  label: string;
  href?: string;
...
const Sidebar = () => {
  ...
  const shouldPreferCollapsed = () => {
    return routesThatPreferCollapsed.some((route) => pathname.startsWith(route));
  };
  ...
  const createNewRecordItems: MenuItem[] = [
    { icon: <ConeIcon className="w-5 h-5" color="currentColor" />, label: "General & Traffic Offence Report", href: "/create-record/general-traffic" },
    { icon: <Gauge className="w-5 h-5" />, label: "Static Speed Check Report", href: "/create-record/static-speed" },
    { icon: <ClipboardList className="w-5 h-5" />, label: "MP Occurrence & Investigation Report", href: "/create-record/mp-investigation" },
  ];

  const reportsAndAnalysis: MenuItem[] = [
    { icon: <Files className="w-5 h-5" />, label: "All Reports", href: "/reports" },
    { icon: <BookOpen className="w-5 h-5" />, label: "Registers/Books", href: "/analysis/registers-books" },
    { icon: <LineChart className="w-5 h-5" />, label: "MP Offence Analysis Monthly Report", href: "/analysis/mp-offence-monthly" },
  ];
      ...
      <Link
        ...
        className={cn(
          ...
          isCollapsed
            ...
            : cn("gap-3 px-4 py-2 hover:bg-gray-100 text-left", isSubmenu && "py-1.5", active && "bg-blue-50 text-blue-700 hover:bg-blue-50")
        )}
      >
        {!isSubmenu && (
          <span className={cn("flex-shrink-0", active ? "text-blue-700" : "text-gray-500 group-hover:text-gray-900")}>
            {item.icon}
          </span>
        )}
        {!isCollapsed && (
          <>
            <span className={cn("flex-1 whitespace-normal break-words leading-snug", isSubmenu ? "text-sm" : "text-base", active ? "text-blue-700 font-medium" : "text-gray-600 group-hover:text-gray-900")}>
              {item.label}
            </span>
            {item.badge && (
      ...
      >
        <CollapsibleTrigger
          className={cn(
            ...
            isCollapsed ? "h-10 justify-center hover:bg-gray-100" : "gap-3 px-4 py-2.5 text-left hover:bg-gray-100",
            hasActiveSubmenu && "bg-blue-50"
          )}
        >
          <span className={cn("flex-shrink-0", hasActiveSubmenu ? "text-blue-700" : "text-gray-500 group-hover:text-gray-900")}>
            {item.icon}
          </span>
          {!isCollapsed && (
            <>
              <span className={cn("flex-1 text-base leading-snug", hasActiveSubmenu ? "text-blue-700 font-medium" : "text-gray-600 group-hover:text-gray-900")}>
                {item.label}
              </span>
              {openMenus.includes(item.label) ? <ChevronDown className="w-4 h-4 text-gray-400" /> : <ChevronRight className="w-4 h-4 text-gray-400" />}
            </>
          )}
        </CollapsibleTrigger>
        {!isCollapsed && (
  ...
  return (
    <div className={cn("h-screen bg-white border-r border-gray-200 flex flex-col transition-all duration-300", isCollapsed ? "w-[74px]" : "w-[340px]")}>
      ...
      <nav className={cn("flex-1 overflow-y-auto no-scrollbar", isCollapsed ? "px-2 pt-4" : "p-4 space-y-1")}>
        <div className="mb-2">
          {menuItems.map((item) => (
        ...
        <div className="mb-2">
          {!isCollapsed && <h2 className="px-4 py-3 text-sm font-bold text-gray-400">Create New Record</h2>}
        ...
        <div className="w-full h-px bg-gray-100 my-2" />
        <div className="mb-2">
          {!isCollapsed && <h2 className="px-4 py-3 text-sm font-bold text-gray-400">Reports & Analysis</h2>}
        ...
        <div className="w-full h-px bg-gray-100 my-2" />
        <div className="pb-4">
          {!isCollapsed && <h2 className="px-4 py-3 text-sm font-bold text-gray-400">System Setup</h2>}
      ...
      <div className="p-4 border-t border-gray-200">
        {!isCollapsed ? (
          <div className="space-y-3">
            ...
            <button onClick={logout} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
              <LogOut className="w-4 h-4" />
              <span>Logout</span>
            </button>
          </div>
        ) : (
          <button onClick={logout} className="w-full flex items-center justify-center p-2 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Logout">
            <LogOut className="w-4 h-4" />
          </button>
        )}
      </div>
