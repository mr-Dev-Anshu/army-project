...
export interface ParticularsBlock {
  ...
  children?: ParticularsBlock[]; // ✅ nested blocks support (very important)
}
...
export const buildBlocksFromIndividuals = (
  individuals: any[],
): ParticularsBlock[] => {

  const blocks: ParticularsBlock[] = [];

  if (!Array.isArray(individuals)) return blocks;
  ...
  individuals.forEach((ind) => {
    ...
    const fields = [
      ...
      { label: "Army No.", value: d.armyNo || d.armyNumber },
      { label: "Unit", value: d.unit },
      { label: "FMN", value: d.fmn },
      { label: "Address", value: d.address },
      { label: "Aadhar", value: d.aadharCardNo },
      { label: "Relation", value: d.relation },
      { label: "Pass No", value: d.passNo },
      { label: "Department", value: d.Department },
      { label: "Employee ID", value: d["Employee ID"] },
    ]
...
export const normalizeParticulars = (
  ...
  // ✅ NEW dynamic individuals support
  if ((particulars as any)?.individuals) {
    return buildBlocksFromIndividuals((particulars as any).individuals);
  }

  // ✅ blocks priority
  if (particulars.blocks?.length) {
