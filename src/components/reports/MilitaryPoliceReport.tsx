...
const MilitaryPoliceReport: React.FC<MilitaryPoliceReportProps> = ({
  ...
  // ✅ supports old + edit preview data
  const primary =
    (particulars as any)?.primary ||
    (particulars as any)?.persons?.[0] ||
    {};

  const secondary =
    (particulars as any)?.secondary ||
    (particulars as any)?.persons?.[1];

  const vehicle = (particulars as any)?.vehicle;

  // ✅ signature safety
  const safeWitnessSig = witnessSig || {
    armyNo: "",
    rank: "",
    name: "",
    unit: "",
  ...
  return (
    <div
      id="mp-report"
      className={`font-[Arial] flex flex-col gap-8 items-center ${className || ""}`}
    >
      {(Object.values(primary).some((v) => v) || secondary || vehicle) && (
        <div className="max-w-[210mm] w-full bg-white p-10">
          ...
          {/* PARTICULARS */}
          <h2 className="font-bold mb-2">1. PARTICULARS</h2>
          <DataField label="Name" value={primary.name} />
          <DataField label="Army No" value={primary.armyNo} />
          <DataField label="Rank" value={primary.rank} />
          <DataField label="Unit" value={primary.unit} />
          <DataField label="FMN" value={primary.fmn} />
          <DataField label="Address" value={primary.address} />
          <DataField label="I Card No" value={primary.iCardNo} />

          {secondary && (
            <>
              <hr className="my-3" />
              <DataField label="Co-Driver Name" value={secondary.name} />
              <DataField label="Army No" value={secondary.armyNo} />
            </>
          )}
          ...
          {vehicle && (
            <>
              <hr className="my-3" />
              <DataField label={vehicle.vehicleNumber} value={vehicle.baNo} />
              <DataField label="Make & Take" value={vehicle.makeAndTake} />
            </>
          )}

          {/* OCCURRENCE */}
          <hr className="my-4" />
          <h2 className="font-bold">2. OCCURRENCE</h2>
          ...
          {/* OFFENCE */}
          <hr className="my-4" />
          <h2 className="font-bold">3. OFFENCE</h2>
          <p>{offence.types?.join(", ")}</p>
          ...
          <div className="flex justify-between">
            <div>
              <b>Witness</b>
              <div>{safeWitnessSig.name}</div>
            </div>

            <div>
              <b>MP</b>
              <div>{safeMpSig.name}</div>
            </div>
          </div>
