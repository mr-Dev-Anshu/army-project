  ...
  return (
    <div>
      {people.map((person, personIndex) => (
        <section
          key={personIndex}
          className="space-y-4 border p-6 rounded-md bg-white shadow-sm"
        >
          {/* ================= HEADER ================= */}
          ...
          {data.individualType === "civilian" && (
            <div className="space-y-4 pt-4">
              ...
              <div className="flex items-center gap-2 pt-2">
                <Checkbox
                  checked={data.individualDetails?.hasRelation || false}
                  onCheckedChange={(v) =>
                    onChange("individualDetails.hasRelation", v)
                  }
                />
                <span className="text-sm font-medium">
                  Related to Military / Employee / Others?
                </span>
              </div>

              {/* 🔥 RELATION INPUT */}
              {data.individualDetails?.hasRelation && (
                <SuggestionInput
                  label="Relation"
                  fieldType="relation"
                  placeholder="Father / Wife / Son"
                  value={data.individualDetails?.relation || ""}
                  onChange={(v) => onChange("individualDetails.relation", v)}
                />
              )}
                ...
                <div className="mt-4 p-4 border rounded-md bg-gray-50 space-y-4">
                  ...
                  {data.individualDetails?.relatedPerson?.individualType && (
                    <IndividualVictimDetails
                      hideHeader
                      hideFooter
                      hideTypeSelector // 🔥 MOST IMPORTANT LINE
                      data={data.individualDetails.relatedPerson}
                      onChange={(path, value) =>
                        onChange(
                          `individualDetails.relatedPerson.${path}`,
                          value,
                        )
                      }
                    />
      ...
      {!hideFooter && (
        <div className="flex items-center justify-end">
          <div
            className="text-right text-white text-sm cursor-pointer bg-black w-fit py-2 px-4 rounded-md mt-4"
            onClick={() =>
              setPeople((prev) => [
                ...prev,
                {
